...
{{
  ...
  "recommendedEvidence": [
  "증거 자료명을 한 항목에 정확히 하나만 작성한다. 괄호 안에 여러 자료를 나열하거나, 쉼표·'등'으로 묶는 것을 금지한다. preparedEvidenceList가 존재하면 해당 목록에 이미 포함된 자료는 제외하고, 추가로 확보하면 유리한 새로운 증거만 추천한다. 예시 - 올바름: ['직원 교육일지', '신분증 확인 절차 개선 안내문', '매출장부'] / 잘못됨: ['시정조치 증빙 자료 (직원 교육일지, 신분증 확인 절차 개선 안내문 등)']"
  ]
  }}
