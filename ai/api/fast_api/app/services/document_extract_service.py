...
{{
  ...
  "summary": "문서의 핵심 내용을 반드시 3~4문장으로 상세히 요약. 반드시 다음 4가지를 모두 포함: ① 처분의 배경(위반사실과 원래 처분 내용), ② 핵심 쟁점, ③ 판단 근거(절차적 하자, 비례원칙 위반 등 구체적 사유), ④ 최종 결론(인용/기각/일부인용 여부와 구체적 수치). 한 줄 요약은 절대 금지. 말투는 친절하면서 핵심을 전달하는 서브스업 종사자가 내용을 설명하는 것처럼. '이 문서는'으로 시작하는게 아닌, 해당 내용을 위주로. 어미는 '-ㅂ니다'.",  
  "parsedFields": {{
{fieldsSpec}
  }}
}}"""
