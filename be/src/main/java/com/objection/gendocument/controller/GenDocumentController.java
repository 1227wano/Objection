...
public class GenDocumentController {
    ...
    public ResponseEntity<ApiResponse<DocumentResponse>> getDocument(
        ...
        DocumentResponse response = genDocumentService.getDocument(
                analysisNo, userDetails.getUserNo(), documentType);
    ...
    public ResponseEntity<ApiResponse<DocumentResponse>> patchDocument(
        ...
        DocumentResponse response = genDocumentService.patchDocument(
                analysisNo, userDetails.getUserNo(), documentType, contentJsonPatch);
    ...
    // 문서 다운로드 (PDF)
    @GetMapping("/{documentType}/download")
    public ResponseEntity<byte[]> downloadDocument(
        ...
        String filename = "APPEAL_CLAIM".equals(documentType) ? "행정심판청구서.pdf" : "보충서면.pdf";
